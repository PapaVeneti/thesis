...
ocp_model.set('name', [model.name,'_disp']);
...
y_expr = [actual_torque;input_torque23; loop_closure;penalize_constr; velocities];
y_ref  = [tau_mh_ref ; 0;0;zeros(2,1);pen_ref;vel_ref];
W = diag( [Wtrack,Wu,Wclosure,Wlim,Wvel]); 
...
% Q = diag( [100,100,100,100,100,10,10,10,10,10 ] )/100;
% 
% 
% %2. Guess for reset 
% xguess= [0,-1,-1];      %reset  -> /100
% xright = consistent_x0([    -pi/3, 1, 1],[0;0;0]);
% xleft  = consistent_x0([-pi/3,-1,-1],[0;0;0])
...
%extra options:
%globalization: 
% ocp_opts.set('nlp_solver_step_length', 0.5);
...
for i=1:N  
    switch reference_mode
        case 'pitch'
%             Wu = [0.5,0.5];
            Wtrack_i(1) = Kgain(1)*(N-i + 0.5) * DT^2/ixx;
            Wtrack_i(2) = Kgain(2)*(N-i + 0.5) * DT^2/izz;
            Wtrack_i(3) = Kgain(3)*(N-i + 0.5) * DT^2/iyy; 
...
%2nd way: star shaped
% Ncutoff = floor( N*0.25);
% Nrest   = N-3*Ncutoff;
% x_traj_init = zeros(nx,N+1);
% %for x0
% x_retrun1 = consistent_x0([-pi/3,1,1]  ,[-1;0;0] );
% x_retrun2 = consistent_x0([-1.9,0,0]   ,[0;-5;-5]);
% x_retrun3 = consistent_x0([-pi/3,-1,-1],[1;0;0]  );
% 
% %for x0 = [0,1,1]
% % x_retrun1 = consistent_x0([-1.9,0,0]   ,[0;-5;-5]);
% % x_retrun2 = consistent_x0([-pi/3,-1,-1],[1;0;0] );
% % x_retrun3 = consistent_x0([0,0,0]      ,[0;1;1]  );
% 
% %---
% 
% steps = Ncutoff ;                             %// number of steps
% x_traj_init(:,1:Ncutoff) = bsxfun(@plus,((x_retrun1(:)-x0(:))./(steps-1))*[0:steps-1],x0(:));
% x_traj_init(:,  Ncutoff+1:2*Ncutoff) = bsxfun(@plus,((x_retrun2(:)-x_retrun1(:))./(steps-1))*[0:steps-1],x_retrun1(:));
% x_traj_init(:,2*Ncutoff+1:3*Ncutoff) = bsxfun(@plus,((x_retrun3(:)-x_retrun2(:))./(steps-1))*[0:steps-1],x_retrun2(:));
% 
% steps = Nrest+1 ;
% x_traj_init(:,3*Ncutoff+1:end) = bsxfun(@plus,((x0(:)-x_retrun3(:))./(steps-1))*[0:steps-1],x_retrun3(:));
...
TMH = base_disp_calc(N,DT,xtraj(1,1:end-1)',utraj');
