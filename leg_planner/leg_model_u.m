...
nx = nx+nu;
sym_x    = vertcat(q, qt,tau);
sym_xdot = SX.sym('xdot', nx, 1);

%input -> rate of input: 
dtau = SX.sym('dtau',nu,1);         
sym_u = dtau;
...
expr_f_expl = vertcat(derivatives,dynamics,rates);
...
%torque
state_constraints(11,:) = [-10  ,10 ]; 
state_constraints(12,:) = [-10  ,10 ];     
state_constraints(13,:) = [-10  ,10 ];

%2. input constraints -> rate of u
input_constraints = 2*[100;100;100]*[-1,1];

%3. kinematic loop closure constraint
path_constraints = HQ;

%% 6.  populate structure:
model.name = 'ntnu_leg_u';
